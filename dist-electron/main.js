...
function registerShortcuts(win2) {
  ...
  const moveWindow = (pos) => {
    ...
    const { width: winW, height: winH } = win2.getBounds();
    const margin = 20;
    ...
    win2.setPosition(Math.round(x), Math.round(y), true);
  };
  ...
  globalShortcut.register("CommandOrControl+Shift+A", () => {
    ...
    win2 == null ? void 0 : win2.webContents.send("trigger-problem-assistant");
  });
}
function unregisterShortcuts() {
  globalShortcut.unregisterAll();
}
async function captureScreen(win2) {
  ...
  try {
    ...
    if (!isSpecializedSearch) {
      ...
      return {
        ...
        sources: []
        // No real-time web sources for general AI query
      };
    }
    let searchFilter = "";
    ...
    const searchQuery = isInstitutionRelevant ? `${cleanQuery} ${searchFilter} "${institutionName}"` : `${cleanQuery} ${searchFilter}`;
    ...
    const sourcesContext = results.map((r, i) => `Source [${i + 1}]: ${r.title}
URL: ${r.url}
...
function setupIpc(win2) {
  ...
  ipcMain.on("set-ignore-mouse", (_event, ignore) => {
    win2 == null ? void 0 : win2.setIgnoreMouseEvents(ignore, { forward: true });
  });
...
async function extractEventsWithGemini(text, imageBuffer = null) {
    ...
    if (containsDeadline(text)) {
      ...
      if (browserWindow && !browserWindow.isDestroyed()) {
        if (!browserWindow.isVisible()) {
          ...
          const { width: screenWidth } = primaryDisplay2.workAreaSize;
